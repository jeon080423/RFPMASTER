...
아래 5가지 섹션에 맞춰 분석 결과를 출력하되, 각 항목은 구체적인 근거(페이지 또는 조항)를 바탕으로 작성하세요.
...
**제안요청서 내 '평가항목 및 배점표'를 분석하여 가장 점수 비중이 높거나 까다로운 '핵심 승부처' 3가지를 선정하세요.**
1. **최고 배점 항목 공략:**
2. **차별화 포인트:**
3. **감점 방지 대책:**

## 3. 과업 내용 기반 필수 수행 체크리스트 (Must-Do List)
**과업지시서 텍스트 곳곳에 흩어져 있는 '반드시 수행해야 할 과업'을 추출하여 체크리스트로 정리하세요.**
* [ ] **착수 및 보고:**
* [ ] **조사 수행:**
* [ ] **데이터 처리:**
* [ ] **안전 및 보안:**
...
**입찰 공고와 작성 지침을 기준으로 '서류 미비'나 '규격 위반'으로 인한 탈락을 방지하기 위한 점검표입니다.**
* **[필수 서류]:** 입찰참가신청서, 제안서(정성/정량), 가격입찰서, 법인등기부등본, 사업자등록증, 신용평가등급확인서, 용역수행실적증명서, 청렴계약서약서
* **[규격 점검]:**
    * [ ] **분량 제한:**
    * [ ] **익명성(블라인드):** (제안서 내 업체명, 로고, 대표자명 삭제 또는 마스킹 처리 필수 여부 확인 [중요])
    * [ ] **파일 형식:**

## 5. 상세 전략 및 가점 요인 (Bonus Strategy)
1. **정량평가 만점 전략:** 경영상태(신용등급 기준) 및 유사 사업 실적 인정 기간/금액 확인.
2. **가점 항목 발굴:** (예: 지역 업체 공동수급 시 가점, 여성/장애인/사회적 기업 가점 여부 확인)
3. **제안 목차 구성안:** 제안요청서에서 요구하는 목차(Index)를 제시하고, 각 목차별로 반드시 포함되어야 할 핵심 키워드를 추천하세요.

---

# Fallback Logic
직전 자료가 없으면 비교 분석을 건너뛰지 말고, 금년도 자료를 심층 분석하여 "단독 분석 모드"로 표를 채우세요.

# Safety & Security
최근 공공 입찰에서 '중대재해처벌법 관련 안전관리계획'과 '개인정보보호(가명처리 등)'가 중요 평가 요소이므로 해당 내용을 반드시 체크리스트에 포함시키세요.
"""
                    user_content = f"[금년도 문서]\n{full_current_text[:8000]}\n\n[직전 연도 문서]\n{prev_text[:4000] if prev_text else '없음'}"
